...
const EditProduct = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        toast.error(error.customMessage || "Không tai duoc thong tin san pham");
      } finally {
        setLoading(false);
      }
    };
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      ...
      toast.success("Cập nhật san pham thanh cong");
      navigate("/admin/product");
    } catch (error) {
      toast.error(error.customMessage || "Không cap nhat duoc san pham");
    }
  };
  ...
  return (
    <AdminLayout title="Chinh sua san pham" subtitle={`Ma san pham: ${id}`}>
      {loading ? (
        <div className="adm-loading">Đang tải du lieu san pham...</div>
      ) : (
