...
const Customers = () => {
  ...
  const paginatedCustomers = filteredCustomers.slice((safeCurrentPage - 1) * PAGE_SIZE, safeCurrentPage * PAGE_SIZE);
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-3 sm:p-4 md:p-6">
          ...
          <div className="relative z-10 flex max-h-[92vh] w-full max-w-3xl flex-col overflow-hidden rounded-[2rem] sm:rounded-[2.5rem] bg-surface shadow-2xl animate-in zoom-in-95 duration-200">
            <div className="flex items-start justify-between gap-4 border-b border-border px-5 py-5 sm:px-8 sm:py-6">
              <div>
                <h2 className="text-xl sm:text-2xl font-black text-text tracking-tight">Thêm khách hàng mới</h2>
                <p className="mt-1 text-xs sm:text-sm font-medium text-text-muted">Biểu mẫu được tối ưu để nhập liệu tốt trên cả mobile và desktop.</p>
              </div>
            ...
            <form onSubmit={handleAddCustomer} className="flex-1 overflow-y-auto px-5 py-5 sm:px-8 sm:py-6">
              <div className="grid grid-cols-1 gap-5 sm:gap-6 md:grid-cols-2">
                <div className="space-y-2 md:col-span-2">
                  <label className="ml-1 text-xs font-black uppercase tracking-wider text-text-muted">Họ và tên</label>
                  <input
                    required
                    type="text"
                ...
                <div className="space-y-2">
                  <label className="ml-1 text-xs font-black uppercase tracking-wider text-text-muted">Số điện thoại</label>
                  <input
                    type="tel"
                ...
                <div className="space-y-2">
                  <label className="ml-1 text-xs font-black uppercase tracking-wider text-text-muted">Mật khẩu</label>
                  <input
                    required
                    type="password"
                ...
                <div className="space-y-2">
                  <label className="ml-1 text-xs font-black uppercase tracking-wider text-text-muted">Vai trò</label>
                  <select
