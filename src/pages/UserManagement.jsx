...
const UserManagement = () => {
  ...
  useEffect(() => {
    const fetchUsers = async () => {
      try {
        ...
        toast.error(error.customMessage || "Không tai duoc danh sach nguoi dung");
      } finally {
        setLoading(false);
      }
    };
  ...
  const handleUpdatePassword = async (userId) => {
    ...
    if (!password || password.length < 6) {
      toast.warning("Mật khẩu moi toi thieu 6 ky tu");
      return;
    }
    ...
    try {
      ...
      toast.success(`Cập nhật mat khau cho user #${userId} thanh cong`);
    } catch (error) {
      toast.error(error.customMessage || "Cập nhật mat khau that bai");
    }
  };

  const handleDelete = async (userId) => {
    if (!window.confirm("Ban co chac chan muon xóa user nay?")) {
      return;
    }
    ...
    try {
      ...
      toast.success(`Đã xóa user #${userId}`);
    } catch (error) {
      toast.error(error.customMessage || "Không thể xóa user");
    }
  };
  ...
  const handleCreateUser = async (event) => {
    ...
    try {
      ...
      toast.success("Tạo user moi thanh cong");
    } catch (error) {
      toast.error(error.customMessage || "Không thể tao user moi");
    }
  };
  ...
  return (
    <AdminLayout title="Quản lý nguoi dung" subtitle="Tạo user, doi mat khau va xóa tai khoan">
      ...
      {loading ? (
        <div className="adm-loading">Đang tải danh sach user...</div>
      ) : (
        <div className="adm-table-wrap">
          <div className="adm-toolbar adm-toolbar-inner">
            <input
              type="text"
              value={search}
              placeholder="Tìm theo ten/email..."
              onChange={(event) => {
