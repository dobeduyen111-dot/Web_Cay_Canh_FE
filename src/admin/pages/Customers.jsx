  ...
  return (
    <div className="space-y-6 md:space-y-8 animate-in fade-in duration-700">
      ...
      <div className="bg-surface rounded-[2rem] border border-border shadow-sm overflow-hidden">
        ...
        <div className="flex flex-col gap-4 border-t border-border px-6 py-4 md:flex-row md:items-center md:justify-between md:px-8">
          <p className="text-xs font-bold text-text-muted">
            Hiển thị {filteredCustomers.length === 0 ? 0 : (currentPage - 1) * PAGE_SIZE + 1}-
            {Math.min(currentPage * PAGE_SIZE, filteredCustomers.length)} / {filteredCustomers.length} khách hàng
          </p>
          <div className="flex items-center gap-2 self-start md:self-auto">
            <button
              ...
              className="rounded-xl border border-border px-4 py-2 text-xs font-black text-text transition-colors hover:bg-slate-50 disabled:cursor-not-allowed disabled:opacity-40"
            >
              Trước
            </button>
