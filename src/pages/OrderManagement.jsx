...
const OrderManagement = () => {
  ...
  useEffect(() => {
    const fetchOrders = async () => {
      try {
        ...
        toast.error(error.customMessage || "Không tai duoc danh sach don hang");
      } finally {
        setLoading(false);
      }
    };
  ...
  const handleUpdateStatus = async (orderId) => {
    ...
    try {
      ...
      toast.success(`Đã cap nhat trang thai don #${orderId}`);
    } catch (error) {
      toast.error(error.customMessage || "Cập nhật trang thai that bai");
    }
  };

  const handleDelete = async (orderId) => {
    if (!window.confirm("Ban co chac chan muon xóa don hang nay?")) {
      return;
    }
    ...
    try {
      ...
      toast.success(`Đã xóa don #${orderId}`);
    } catch (error) {
      toast.error(error.customMessage || "Không thể xóa don hang");
    }
  };
  ...
  return (
    <AdminLayout title="Quản lý don hang" subtitle="Cập nhật trang thai va xóa don khong hop le">
      <div className="adm-toolbar">
        <input
          type="text"
          value={search}
          placeholder="Tìm theo ma don/ten khach..."
          onChange={(event) => {
      ...
      {loading ? (
        <div className="adm-loading">Đang tải danh sach don hang...</div>
      ) : (
