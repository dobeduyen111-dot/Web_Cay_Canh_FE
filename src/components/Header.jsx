...
import { WISHLIST_UPDATED_EVENT } from "../utils/wishlistEvents";
...
const Header = () => {
  ...
  useEffect(() => {
    const fetchWishlistCount = async () => {
      ...
      try {
        ...
        setWishlistCount(Array.isArray(response?.items) ? response.items.length : 0);
      } catch (error) {
        setWishlistCount(0);
      }
    };
    ...
    fetchWishlistCount();
    window.addEventListener(WISHLIST_UPDATED_EVENT, fetchWishlistCount);
    return () => window.removeEventListener(WISHLIST_UPDATED_EVENT, fetchWishlistCount);
  }, [isAuthenticated]);
  ...
  const resolveImage = (image) => {
    if (!image) return "https://placehold.co/120x120?text=Cay+Canh";
  ...
  const handleModalLogin = async (event) => {
    ...
    try {
      ...
      navigate(isAdminRole(userProfile?.role) ? "/admin/dashboard" : "/", { replace: true });
    } catch (error) {
      toast.error(
        `${error.customMessage || "Đăng nhập thất bại."}${error.customErrorCode ? ` [${error.customErrorCode}]` : ""}`
      );
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-950/60 p-4">
          <div className="grid w-full max-w-4xl overflow-hidden rounded-[2rem] bg-white shadow-2xl lg:grid-cols-[1.05fr_1.25fr]">
            <div className="bg-[linear-gradient(160deg,_#052e16,_#065f46_55%,_#34d399)] p-8 text-white sm:p-10">
              <p className="mb-3 text-sm uppercase tracking-[0.28em] text-emerald-100">Cây Cảnh</p>
