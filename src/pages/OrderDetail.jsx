        ...
        <div className="order-progress-card">
          <div className="order-progress-head">
            ...
            {status.tone === "cancelled" ? (
              ...
              <div className={`order-progress-current ${status.tone}`}>
                <span>Hiện tại</span>
          ...
          <div className={`order-progress-track ${status.tone === "cancelled" ? "cancelled" : ""}`}>
            {ORDER_STEPS.map((step, index) => {
              ...
              const isUpcoming = status.tone !== "cancelled" && stepNumber > status.progress;
              const hasConnector = index < ORDER_STEPS.length - 1;
              const isConnectorActive =
                status.tone !== "cancelled" && stepNumber < status.progress;
              ...
              return (
                ...
                >
                  {hasConnector ? (
                    <div
                      className={`order-progress-connector ${isConnectorActive ? "active" : ""}`}
                      aria-hidden="true"
                    ></div>
                  ) : null}
            ...
            <div className="order-detail-card">
              ...
              {items.length === 0 ? (
                <div className="order-detail-empty">
                  Hệ thống chưa trả về danh sách sản phẩm cho đơn hàng này.
                </div>
              ) : (
