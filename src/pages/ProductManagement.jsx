...
const ProductManagement = () => {
  ...
  const fetchProducts = useCallback(async () => {
    try {
      ...
      toast.error(error.customMessage || "Không tai duoc danh sach san pham");
    } finally {
      setLoading(false);
    }
  }, []);
  ...
  const handleDelete = async (id) => {
    ...
    try {
      ...
      toast.success("Xóa san pham thanh cong");
    } catch (error) {
      toast.error(error.customMessage || "Không thể xóa san pham");
    }
  };
  ...
  return (
    <AdminLayout title="Quản lý san pham" subtitle="Theo doi ton kho va cap nhat thong tin">
      <div className="adm-toolbar">
        <input
          type="text"
          value={search}
          placeholder="Tìm theo ten san pham..."
          onChange={(event) => {
      ...
      {loading ? (
        <div className="adm-loading">Đang tải danh sach san pham...</div>
      ) : (
