  ...
  const handleSubmitProduct = async (event) => {
    ...
    try {
      ...
      const payload = {
        ...
        image: productForm.imageFile || (productForm.imageUrl.trim() ? productForm.imageUrl.trim() : null),
      ...
      const mappedProduct = {
        ...
        image: response.image || productForm.imageUrl || undefined,
        categoryId,
    ...
    <div className="space-y-6 md:space-y-8 animate-in fade-in duration-700">
      ...
      <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
        <p className="text-xs font-bold text-text-muted">
          Hiển thị {products.length === 0 ? 0 : (currentPage - 1) * PAGE_SIZE + 1}-
          {Math.min(currentPage * PAGE_SIZE, products.length)} / {products.length} sản phẩm
        </p>
        <div className="flex items-center gap-2 self-start md:self-auto">
          <button
            ...
            className="rounded-xl border border-border bg-white px-4 py-2 text-xs font-black text-text transition-colors hover:bg-slate-50 disabled:cursor-not-allowed disabled:opacity-40"
          >
            Trước
          </button>
          <span className="min-w-[92px] text-center text-xs font-black text-text-muted">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 items-start">
                  ...
                  <div className="sm:col-span-1">
                    <div className="aspect-square rounded-2xl border border-border bg-slate-50 overflow-hidden">
                      <img
                        src={imagePreviewUrl || productForm.imageUrl || fallbackImage}
                        alt="preview"
