...
const ForgotPassword = () => {
  ...
  const handleSendOtp = async (event) => {
    ...
    try {
      ...
      toast.error(`${error.customMessage || "Không thể gui OTP."}${error.customErrorCode ? ` [${error.customErrorCode}]` : ""}`);
    } finally {
      setSubmitting(false);
    }
  };
  ...
  const handleVerifyOtp = async (event) => {
    ...
    try {
      ...
      toast.success(response?.message || "Xác thực OTP thanh cong.");
    } catch (error) {
  ...
  const handleResetPassword = async (event) => {
    ...
    if (newPassword !== confirmPassword) {
      toast.error("Mật khẩu nhap lai khong khop.");
      return;
    }
    ...
    try {
      ...
      toast.error(
        `${error.customMessage || "Không thể cap nhat mat khau moi."}${error.customErrorCode ? ` [${error.customErrorCode}]` : ""}`
      );
    } finally {
      setSubmitting(false);
    }
  ...
  const handleResendOtp = async () => {
    ...
    try {
      ...
      toast.error(`${error.customMessage || "Không thể gui lai OTP."}${error.customErrorCode ? ` [${error.customErrorCode}]` : ""}`);
    } finally {
      setSubmitting(false);
    }
  };
    ...
    <section className="page-section">
      <div className="mx-auto grid max-w-5xl overflow-hidden rounded-[2rem] bg-white shadow-[0_30px_80px_rgba(15,23,42,0.08)] lg:grid-cols-[0.95fr_1.05fr]">
        <div className="bg-[linear-gradient(150deg,_#052e16,_#065f46_55%,_#d1fae5)] p-8 text-white sm:p-12">
          <p className="text-sm font-semibold uppercase tracking-[0.28em] text-emerald-100">Quên mật khẩu</p>
          <h1 className="mt-4 text-4xl font-semibold leading-tight">Đặt lại mật khẩu bằng mã OTP được gửi qua email.</h1>
        ...
        <div className="p-8 sm:p-12">
          ...
          <p className="mt-3 text-sm leading-6 text-slate-500">
            Hãy sử dụng mã OTP 6 chữ số để xác minh trước khi cập nhật mật khẩu.
          </p>
          ...
          {step === STEPS.EMAIL && (
            <form onSubmit={handleSendOtp} className="mt-8 space-y-5">
              ...
              <button type="submit" disabled={submitting} className="primary-button w-full">
                {submitting ? "Dang gui OTP..." : "Gửi OTP"}
              </button>
            </form>
          )}
          ...
          {step === STEPS.OTP && (
            <form onSubmit={handleVerifyOtp} className="mt-8 space-y-5">
              ...
              <button type="submit" disabled={submitting} className="primary-button w-full">
                {submitting ? "Dang xac thuc..." : "Xác thực OTP"}
              </button>
          ...
          {step === STEPS.PASSWORD && (
            <form onSubmit={handleResetPassword} className="mt-8 space-y-5">
              ...
              <div>
                <label className="field-label" htmlFor="reset-password">Mật khẩu moi</label>
                <input
                  id="reset-password"
                  type="password"
                  className="field-input"
              ...
              <button type="submit" disabled={submitting} className="primary-button w-full">
                {submitting ? "Dang cap nhat..." : "Cập nhật mat khau"}
              </button>
            </form>
          )}
        </div>
